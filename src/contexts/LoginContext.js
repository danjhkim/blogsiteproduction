...
export class LoginStore extends React.Component {
    ...
    onAuthChange = (isSignedIn) => {
        if (isSignedIn) {
            this.signIn(this.auth.currentUser.get().getId(), this.auth.currentUser.ee.Is.sd);
        } else {
            this.signOut()
        }
        // this.auth.isSignedIn.get() ? alert('Logged In') : alert('Logged Out')
    }
    ...
    render() {
        return (
            <Context.Provider value={{ ...this.state, signOut: this.signOut, signIn: this.signIn, loadAuth: this.loadAuth, onSignInClick: this.onSignInClick, onSignOutClick: this.onSignOutClick, checkBox: this.checkBox }}>
                {this.props.children}
            </Context.Provider>
        )
    }
