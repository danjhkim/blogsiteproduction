...
// THIS USES A REGULAR FORM NOT REDUX FORM
	...
	if (complete) {
		return (
			<Modal
				...
				title={`Your blog post has been ${completed}!`}
			/>
		);
	}
				...
				<form onSubmit={handleSubmit}>
					...
					<div className={styles.buttonContainer}>
						<div className={styles.checkbox}>
							...
							<input
								defaultChecked={initalData.showPost}
								className={styles.checking}
								type='checkbox'
								onChange={() => gID.checkBox()}
							/>
						</div>
						{!pending ? (
							<button
								className={`${styles.button} ${animations.hvrgrow}`}
								type='submit'>
								{buttonText}
							</button>
						) : (
